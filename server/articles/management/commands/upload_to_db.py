...
from articles.management.commands.get_articles import get_articles, get_NIPS
...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        for res in articles:
            ...
            if arxiv_id in articles_from_bd:
                article = Article.objects.get(entry_id=arxiv_id)
                article.conference = conf
                self.stdout.write(self.style.SUCCESS(article.title))
                article.save()
                ...
                self.stdout.write(self.style.SUCCESS(arxiv.title))
                article.save()
